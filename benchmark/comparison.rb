...
mass_minting_single_decimal = Benchmark.measure do
  MintingSingleDecimal.transaction { NUM_RECORDS.times { MintingSingleDecimal.create!(price: MINTING_MONEY) } }
end
mass_money_rails_single = Benchmark.measure do
  MoneyRailsSingle.transaction { NUM_RECORDS.times { MoneyRailsSingle.create!(price: MONEY_RAILS_MONEY) } }
end
mass_minting_composite = Benchmark.measure do
  MintingComposite.transaction { NUM_RECORDS.times { MintingComposite.create!(price: MINTING_MONEY) } }
end
mass_minting_composite_decimal = Benchmark.measure do
  MintingCompositeDecimal.transaction { NUM_RECORDS.times { MintingCompositeDecimal.create!(price: MINTING_MONEY) } }
end
mass_money_rails_composite = Benchmark.measure do
  MoneyRailsComposite.transaction { NUM_RECORDS.times { MoneyRailsComposite.create!(price: MONEY_RAILS_MONEY) } }
end

puts format('%-40s %10s', 'minting-rails (single integer):', "#{mass_minting_single.real.round(4)}s")
puts format('%-40s %10s', 'minting-rails (single decimal):', "#{mass_minting_single_decimal.real.round(4)}s")
puts format('%-40s %10s', 'money-rails  (single integer):', "#{mass_money_rails_single.real.round(4)}s")
puts format('%-40s %10s', 'minting-rails (comp integer):', "#{mass_minting_composite.real.round(4)}s")
puts format('%-40s %10s', 'minting-rails (comp decimal):', "#{mass_minting_composite_decimal.real.round(4)}s")
puts format('%-40s %10s', 'money-rails  (comp integer):', "#{mass_money_rails_composite.real.round(4)}s")
